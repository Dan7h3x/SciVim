...
function M.foldtext()
	...
	table.insert(ret, { " " .. require("NvimPy.configs.icons").misc.dots })
...
function M.statuscolumn()
	...
	local show_open_folds = vim.g.statcol and vim.g.statcol.open
	local use_githl = vim.g.statcol and vim.g.statcol.githl
	...
	if show_signs then
		...
		vim.api.nvim_win_call(win, function()
			if vim.fn.foldclosed(vim.v.lnum) >= 0 then
				fold = { text = vim.opt.fillchars:get().foldclose or "󰫍 ", texthl = githl or "Folded" }
			elseif
				show_open_folds
				and not require("NvimPy.utils.statcol").skip_foldexpr[buf]
				-- and vim.treesitter.foldexpr(vim.v.lnum):sub(1, 1) == ">"
				and tostring(vim.treesitter.foldexpr(vim.v.lnum)):sub(1, 1) == "󰫍 "
			then -- fold start
