...
local function process_signature_help_results(results)
	...
	for client_id, r in pairs(results) do
		...
		local client = assert(lsp.get_client_by_id(client_id))
		if err then
			vim.notify(client.name .. ": " .. tostring(err.code) .. ": " .. err.message, vim.log.levels.ERROR)
...
function M.signature_help(config)
	...
	lsp.buf_request_all(0, method, client_positional_params(), function(results, ctx)
		...
		local function show_signature(update_win)
			...
			local lines, hl = util.convert_signature_help_to_markdown_lines({ signatures = { result } }, ft, triggers)
			if not lines then
				return
			end
			...
			if hl then
				...
				vim.hl.range(buf, sig_help_ns, "LspSignatureActiveParameter", { hl[1], hl[2] }, { hl[3], hl[4] })
			end
			return buf, win
		end
...
function M.rename(new_name, opts)
	...
	local function try_use_client(idx, client)
		...
		local function rename(name)
			...
			params.newName = name
			local handler = client.handlers[ms.textDocument_rename] or lsp.handlers[ms.textDocument_rename]
...
function M.add_workspace_folder(workspace_folder)
	workspace_folder = workspace_folder or npcall(vim.fn.input, "Workspace Folder: ", vim.fn.expand("%:p:h"), "dir")
	api.nvim_command("redraw")
...
function M.remove_workspace_folder(workspace_folder)
	workspace_folder = workspace_folder or npcall(vim.fn.input, "Workspace Folder: ", vim.fn.expand("%:p:h"))
	api.nvim_command("redraw")
