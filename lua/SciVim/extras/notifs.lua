...
local function setup_highlights()
	...
	api.nvim_set_hl(0, "Notif", { bg = theme.background, fg = theme.foreground })
	api.nvim_set_hl(0, "NotifBorder", { fg = theme.border })
	api.nvim_set_hl(0, "NotifTitle", { fg = theme.primary, bold = true })
	api.nvim_set_hl(0, "NotifInfo", { fg = theme.info })
	api.nvim_set_hl(0, "NotifWarn", { fg = theme.warning })
	api.nvim_set_hl(0, "NotifError", { fg = theme.error })
	api.nvim_set_hl(0, "NotifSuccess", { fg = theme.success })
	api.nvim_set_hl(0, "NotifTime", { fg = theme.secondary })
	api.nvim_set_hl(0, "NotifActions", { fg = theme.primary })

	-- Control center specific highlights
	api.nvim_set_hl(0, "NotifCenter", { bg = theme.background, fg = theme.foreground })
	api.nvim_set_hl(0, "NotifCenterHeader", { fg = theme.primary, bold = true })
	api.nvim_set_hl(0, "NotifCenterStats", { fg = theme.secondary, italic = true })
	api.nvim_set_hl(0, "NotifCenterSection", { fg = theme.primary, bold = true })
	api.nvim_set_hl(0, "NotifCenterHelp", { fg = theme.secondary })
	api.nvim_set_hl(0, "NotifCenterFilter", { fg = theme.info, bold = true })
	api.nvim_set_hl(0, "NotifCenterEmpty", { fg = theme.secondary, italic = true })
end
...
local function create_control_center()
	...
	api.nvim_win_set_option(win, "winhl", "Normal:NotifCenter,FloatBorder:NotifBorder")
	...
	if notification_state.control_center.is_open and notification_state.control_center.buf then
		...
		for i, line in ipairs(content) do
			...
			if line:match("^🔔.*Control Center") then
				api.nvim_buf_add_highlight(buf, ns_id, "NotifCenterHeader", line_idx, 0, -1)
			-- Stats line
			elseif line:match("^🟢.*Active:") then
				api.nvim_buf_add_highlight(buf, ns_id, "NotifCenterStats", line_idx, 0, -1)
			-- Filter status
			elseif line:match("^🔍") then
				api.nvim_buf_add_highlight(buf, ns_id, "NotifCenterFilter", line_idx, 0, -1)
			-- Section headers
			elseif line:match("^🔔.*Active") or line:match("^📚.*History") then
				api.nvim_buf_add_highlight(buf, ns_id, "NotifCenterSection", line_idx, 0, -1)
			-- Help/commands
			elseif line:match("^💡") or line:match("^%s*%[") then
				api.nvim_buf_add_highlight(buf, ns_id, "NotifCenterHelp", line_idx, 0, -1)
			-- Empty state messages
			elseif line:match("🎉") or line:match("📦") then
				api.nvim_buf_add_highlight(buf, ns_id, "NotifCenterEmpty", line_idx, 0, -1)
			-- Notification entries with level-specific colors
			elseif line:match("🔵" or config.icons.info) then -- Info
				api.nvim_buf_add_highlight(buf, ns_id, "NotifInfo", line_idx, 0, -1)
			elseif line:match("🟡" or config.icons.warn) then -- Warning
				api.nvim_buf_add_highlight(buf, ns_id, "NotifWarn", line_idx, 0, -1)
			elseif line:match("🔴" or config.icons.error) then -- Error
				api.nvim_buf_add_highlight(buf, ns_id, "NotifError", line_idx, 0, -1)
			elseif line:match("🟢" or config.icons.success) then -- Success
				api.nvim_buf_add_highlight(buf, ns_id, "NotifSuccess", line_idx, 0, -1)
			end
		end
...
function M.setup()
	...
	api.nvim_create_user_command("Notif", M.toggle, {
	...
	api.nvim_create_user_command("NotifHistory", function()
		M.open_control_center()
