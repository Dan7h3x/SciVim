...
local utils = require("SciVim.utils")
...
M._load_bps = function()
	...
	utils.info(string.format("Loaded %d breakpoints in %d bufers.", bp_count, vim.tbl_count(path2bufnr)))
end
...
M._store_bps = function()
	...
	if fp then
		...
		fp:close()
		utils.info(string.format("Stored %d breakpoints in %d bufers.", bp_count, vim.tbl_count(breakpoints_by_buf)))
	end
end
...
M.config = function()
	...
	map({ "n", "v" }, "<leader>dc", dap.continue, { silent = true, desc = "DAP launch or continue" })
	map({ "n", "v" }, "<S-F5>", function()
		require("osv").launch({ port = 8086 })
	end, { silent = true, desc = "Start OSV Lua Debug Server" })
	map(
		{ "n", "v" },
		"<leader>du",
		require("SciVim.plugins.debug.ui").toggle,
		{ silent = true, desc = "DAP toggle UI" }
	)
	map({ "n", "v" }, "<S-F8>", function()
		require("plugins.dap.ui").toggle(true, true)
	end, { silent = true, desc = "DAP toggle UI" })
	map({ "n", "v" }, "<leader>dB", require("dap").toggle_breakpoint, { silent = true, desc = "DAP toggle breakpoint" })
	map({ "n", "v" }, "<leader>dso", dap.step_over, { silent = true, desc = "DAP step over" })
	map({ "n", "v" }, "<leader>dsi", dap.step_into, { silent = true, desc = "DAP step into" })
	map({ "n", "v" }, "<leader>dsO", dap.step_out, { silent = true, desc = "DAP step out" })
	map({ "n", "v" }, "<leader>dt", dap.terminate, { silent = true, desc = "DAP Terminate" })

	-- Conditional breakpoints
	map({ "n", "v" }, "<leader>dbc", function()
	...
	map({ "n", "v" }, "<leader>dr", dap.repl.toggle, { silent = true, desc = "DAP toggle debugger REPL" })

	-- DAP-UI widgets
	map({ "n", "v" }, "<Leader>dk", require("dap.ui.widgets").hover, { silent = true, desc = "DAP Hover" })
	map({ "n", "v" }, "<Leader>dp", require("dap.ui.widgets").preview, { silent = true, desc = "DAP Preview" })
	...
	map(
		{ "n", "v" },
		"<leader>d?",
		fzf_lua("dap_commands"),
		{ silent = true, desc = "DAP: fzf nvim-dap builtin commands" }
	)
	map({ "n", "v" }, "<leader>dfb", fzf_lua("dap_breakpoints"), { silent = true, desc = "DAP: fzf breakpoint list" })
	map({ "n", "v" }, "<leader>dff", fzf_lua("dap_frames"), { silent = true, desc = "DAP: fzf frames" })
	map({ "n", "v" }, "<leader>dfv", fzf_lua("dap_variables"), { silent = true, desc = "DAP: fzf variables" })
	map(
		{ "n", "v" },
		"<leader>dfx",
	...
	require("SciVim.plugins.debug.ui").setup()
	require("SciVim.plugins.debug.lua")
	require("SciVim.plugins.debug.python")
