	...
	for _, part in ipairs(conf or {}) do
		...
		elseif part.type == "data" then
			local input = {
				...
				buffer = buffer,
				text = foldStartLine,
				start_line = foldStart,
				end_line = foldEnd,
				foldTextLen = foldtext.get_len(_o),
			}

			if
				type(part.value) == "function" and pcall(part.value --[[@as function]], input)
			then
...
foldtext.setup = function(config_table)
	...
	vim.o.foldtext = "v:lua.require('SciVim.utils.fold').text()"
