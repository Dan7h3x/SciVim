...
function M.ensure_treesitter_cli(cb)
	...
	local function fail(msg)
		return require("SciVim.utils").err({
	...
	mr.refresh(function()
		...
		if not p:is_installed() then
			require("SciVim.utils").info("Installing `tree-sitter-cli` with `mason.nvim`...")
			p:install(
				nil,
				vim.schedule_wrap(function(success)
					if success then
						require("SciVim.utils").info("Installed `tree-sitter-cli` with `mason.nvim`.")
						cb()
					else
