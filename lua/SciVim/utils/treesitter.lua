...
---@return boolean ok, SciVim.util.treesitter.Health health
function M.check()
  ...
  ---@class SciVim.util.treesitter.Health: table<string,boolean>
  local ret = {
...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      SciVim.err(lines, { title = "SciVim Treesitter" })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      SciVim.utils.info("Installing `tree-sitter-cli` with `mason.nvim`...")
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            SciVim.info("Installed `tree-sitter-cli` with `mason.nvim`.")
            cb(true)
          else
